...
1. **Mods & Plugins** → upload `ValheimAutoCleanup-1.0.2.zip`.
2. **Save and Reboot.**
...
```
[Info   :   BepInEx] Loading [Valheim Auto Cleanup 1.0.2]
[Info   :Valheim Auto Cleanup] Valheim Auto Cleanup 1.0.2 loaded. Waiting for the world to start.
...
**Chat commands do nothing.** They are off by default: set `EnableChatCommands = true`.
Then check the account is genuinely in the admin list (the log says when it ignores a
non-admin), and remember the server only sees chat while another player is online. If the
log shows "Could not enable admin chat commands", use the command file instead.
